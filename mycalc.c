...
main(int argc, char *argv[]) {
	...
	// define padr�es de entrada e sa�da (output/input)
	source = stdin;        
	object = stdout;
...
// loop principal de execu��o (interpretador)  
	while(1) {	
		fprintf(object, "%s", PROMPT);  
		double value;				
		if(mycalc(&value)) { // se n�o houver erro, exibe o valor
			fprintf(object, " = %.2f\n", value);
		}
	}
	return EXIT_SUCCESS;
...
// fun��o principal do programa, retorno:
// 1 - SUCESSO
// 0 - FALHOU
mycalc(double *value) {  	  
...
// fun��o que exibe a mensagem referente ao status retornado
...
// fun��o que pode ser chamada externamente para ativar o flag de erro
